...
The `--scene` parameter takes a JSON string. Quote escaping rules differ across shells, and inline
JSON is fragile (especially on Windows). The **safest approach is always to write the payload
to a file and pass `"$(cat file)"` (or its shell equivalent) to the CLI.**

### 1. File-based (Recommended for ALL platforms)

#### bash / zsh (Linux, macOS, Git Bash, WSL)

```bash
# Heredoc avoids any quoting headaches
cat > /tmp/scene.json << 'EOF'
{
  ...
  "LoadConfig": {
    "TestMode": "concurrency_mode",
    "MaxRunningTime": 1,
    "Configuration": { "AllConcurrencyBegin": 10, "AllConcurrencyLimit": 10 }
  }
}
EOF

aliyun pts save-pts-scene --scene "$(cat /tmp/scene.json)"
```

#### PowerShell (Windows / pwsh)

```powershell
# Build the object in PowerShell, dump to JSON, hand off as a single string
$scene = @{
    SceneName    = 'my-test'
    RelationList = @(@{
        RelationName = 'link-1'
...
$scene | ConvertTo-Json -Depth 10 | Set-Content -Path scene.json -Encoding UTF8

aliyun pts save-pts-scene --scene (Get-Content scene.json -Raw)
```

#### cmd.exe (Windows legacy)

```cmd
REM Always use a file. Inline JSON in cmd.exe is extremely fragile.
aliyun pts save-pts-scene --scene "%CD%\scene.json"
REM (some CLI builds also accept @scene.json; verify with --help)
```

### 2. Inline JSON (acceptable on Linux/macOS, avoid on Windows)

#### bash / zsh

```bash
# Single-quote the whole JSON; no inner escaping needed
aliyun pts save-pts-scene --scene '{"SceneName":"x","RelationList":[],"LoadConfig":{}}'
```

#### PowerShell (inline — requires backtick escaping)

```powershell
# Each " must be backtick-escaped. Highly error-prone. PREFER FILE METHOD.
aliyun pts save-pts-scene --scene '{"SceneName":"x","RelationList":[],"LoadConfig":{}}'
```
...
|--------------|--------------|-----|
| `场景名不能为空` / `SceneName cannot be empty` | JSON parse failed; `"SceneName"` arrived empty after shell mangling | Use file-based method |
| `Invalid JSON` | Nested quotes corrupted by shell | Use heredoc (bash) or `Set-Content` (PowerShell) |
| `Unexpected token` from CLI | Trailing CRLF or BOM in file | Save file as UTF-8 (no BOM) and LF line endings |
| Empty `RelationList` reaching API | Outer single-quote got stripped (Windows) | Switch to file-based method |

### 4. Cross-shell quick reference

| Shell | Recommended pattern |
|-------|--------------------|
| bash / zsh | `--scene "$(cat scene.json)"` or `--scene '<inline-json>'` |
| PowerShell | `--scene (Get-Content scene.json -Raw)` |
| cmd.exe | File path / `@scene.json` (inline JSON not supported reliably) |
| Git Bash on Windows | Same as bash, but prefer file method to avoid path-translation surprises |
