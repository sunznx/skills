...
mkdir -p ~/.local/bin
install -m 755 aliyun ~/.local/bin/
# Ensure ~/.local/bin is in your PATH (add to ~/.zshrc or ~/.bash_profile if not):
#   export PATH="$HOME/.local/bin:$PATH"
# If you prefer system-wide install, run the move command with elevated privileges manually.
...
# Extract and install to user-local bin (no sudo required)
