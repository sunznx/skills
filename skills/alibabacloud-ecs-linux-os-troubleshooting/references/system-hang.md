...
1. Follow [cloudmonitor-metrics](utils/cloudmonitor-metrics.md) to backtrack the resource trends (CPU, memory, IO, network) before the hang, and determine whether resource exhaustion caused the hang.
2. Use `DescribeInstanceHistoryEvents` to check whether hang-related system events exist, and determine whether the hang is occasional or frequent.
   - If yes, analyze it based on the event type and severity in the event details.
   - If no, continue.
3. Follow [guestos-console-log](utils/guestos-console-log.md) to obtain the serial console log and search for the `Hang` keywords, such as `soft lockup`, `hung_task`, and `watchdog`.
   - If found, expand the context according to that document and perform root cause analysis.
   - If not found, continue.
4. Check whether the system logs contain hang logs, or whether there are coredump files under /var/crash/.
