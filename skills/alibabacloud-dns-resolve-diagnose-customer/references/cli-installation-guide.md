...
# Move to PATH (user-local, no root required)
...
# Extract and install (user-local, no root required)
...
Credentials should be provided by the runtime environment via the **default credential chain** (pre-set environment variables, CLI profile, RAM role). Do not set AK/SK values manually in commands or scripts.
