...
description: 向一个或多个 agent 发送任务，按需等待回复，并在原生投递明确失败时回退到 Herdr。用户要求联系、通知、委派任务给其他 agent 或等待其结果时使用。
---
...
- `request`：投递后等待接收方回复。用户明确说“等待”“等回复”“拿到结果”等时使用。
- 用户未指定且语义不明确时，默认使用 `notify`。
