...
* What node version is installed. Note: for specified container environments, the node environment needs to be installed through the `Install Node Environment` step
...
The code root directory may have a `.nvmrc` file. If this file exists, the node version in this file takes precedence.
