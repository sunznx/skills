...
Call `yunxiao_flow_get_vm_deploy_order.py` to get detailed information about the Docker deployment order, including deployment status, batch progress, deployment machine list, etc.
...
Find the machineSn of the corresponding machine from Step 1, then call `yunxiao_flow_get_vm_deploy_machine_log.py` to get Docker deployment logs.
