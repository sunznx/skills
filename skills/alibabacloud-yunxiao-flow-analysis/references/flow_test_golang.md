...
* What golang version is installed. Note: for specified container environments, the golang environment needs to be installed through the `Install Go` step
* What is the file path of the unit test report. Whether "Stop pipeline when test cases fail" (FAIL_ON_ERROR) is enabled
* Unit test report file path. Whether "Stop pipeline when test cases fail" (FAIL_ON_ERROR) is enabled
