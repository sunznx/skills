...
export function collectRuleHistory(catalog, repository) {
    const git = (...args) => execFileSync('git', ['--no-optional-locks', '-C', repository, ...args], {
        encoding: 'utf8', windowsHide: true, maxBuffer: 32 * 1024 * 1024,
    }).trimEnd();
    const snapshot = catalog.project.revision;
    if (!/^[a-f0-9]{40}$/.test(snapshot))
        throw new Error('History requires a full snapshot commit');
    git('cat-file', '-e', `${snapshot}^{commit}`);
    const shallow = git('rev-parse', '--is-shallow-repository') === 'true';
    const sourceCache = new Map();
    const historyCache = new Map();
    return { ...catalog, rules: catalog.rules.map(rule => {
            const untracked = (reason) => ({ ...rule, history: { status: 'untracked', reason, snapshot } });
            if (shallow)
                return untracked('shallow-history');
            ...
            if (!source || !Number.isInteger(rule.line) || !Number.isInteger(rule.endLine)
                ...
                throw new Error('Invalid rule source range');
            if (!sourceCache.has(rule.sourcePath))
                sourceCache.set(rule.sourcePath, git('show', `${snapshot}:${rule.sourcePath}`));
            if (sourceCache.get(rule.sourcePath).replace(/\r\n/g, '\n') !== source.text.replace(/\r\n/g, '\n').trimEnd())
                throw new Error('Catalog source differs from Git snapshot');
...
export function validateRuleHistory(history, rule, snapshot) {
    if (!history || history.status === 'untracked')
        return false;
    if (history.status !== 'tracked' || history.method !== 'git-line-history' || history.snapshot !== snapshot
        ...
        || history.lastEdited !== history.commits[0].date)
        throw new Error(`Invalid rule history: ${rule.id}`);
    return true;
}
