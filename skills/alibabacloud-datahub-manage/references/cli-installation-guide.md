...
```bash
aliyun datahub list-projects --profile projectA

export ALIBABA_CLOUD_PROFILE=projectA
aliyun datahub list-projects   # Uses projectA
```
...
```bash
# Basic test - list DataHub projects in a region
aliyun datahub list-projects --region cn-hangzhou

# Expected output: JSON object containing project list (may be empty if no projects exist)
```

**If successful**, you'll see a JSON response similar to:
```json
{
  "ProjectList": [
    {
      "ProjectName": "example_project",
      "Comment": "Sample DataHub project",
...
An empty `ProjectList` is also a valid success — it means authentication works but no projects exist yet.
...
aliyun datahub list-projects --region cn-hangzhou --log-level=debug
...
```bash
# Example: Read-only DataHub access
# Attach policy: AliyunDataHubReadOnlyAccess
# Or use a custom policy with only dhs:List* and dhs:Get* actions
```
...
# Example: AliyunDataHubFullAccess for DataHub operations
```
...
# Check DataHub-supported regions: see https://help.aliyun.com/zh/datahub/
# Or test with a known region
aliyun datahub list-projects --region cn-hangzhou
...
```bash
# Use --endpoint flag to override service endpoint per command
aliyun datahub list-projects --endpoint https://dh-cn-hangzhou.aliyuncs.com
```
...
1. **Install plugins** for services you need (v3.3.3+ supports all published product plugins):
   ```bash
   aliyun plugin install --name datahub
...
2. **Explore commands**:
   ```bash
   aliyun datahub --help
   aliyun datahub create-project --help
   ```

3. **Read documentation**:
   - [Aliyun CLI Official Docs](https://help.aliyun.com/zh/cli/)
   - [DataHub Help Center](https://help.aliyun.com/zh/datahub/)
