...
Before editing or redrawing any source diagram (SVG, image, PDF, draw.io, or other format), create a **source fidelity ledger** of its nodes, connectors, labels, legend items, and semantic notes. Preserve every semantic item unless the user explicitly approves its removal; the complexity budget is not permission to drop source meaning. Recheck the finished diagram against the ledger.
