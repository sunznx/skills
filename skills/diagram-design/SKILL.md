...
2. **Label-to-connector margin: 6–10px gap, always.** A label must never sit *on* its arrow — the connector must remain visible. Place the label centered above (or beside, for vertical segments) the line with a **minimum 6px gap** between the bottom of the label's mask rect and the connector stroke. The opaque mask rect prevents the arrow from bleeding through, but the *visible* gap between mask edge and line preserves the reader's ability to trace the connection. If the label is large enough that 6px feels cramped, push it to 8–10px. Never let the mask rect touch or overlap the stroke.
   - Put the label on a dedicated connector segment outside its endpoint nodes. Its mask must not touch or cover a node border; reroute the connector or add whitespace when no safe segment exists.
...
- [ ] If editing/redrawing an existing diagram, source fidelity ledger lists every node, connector, label, legend item, and semantic note? Final output rechecked against it?
...
- [ ] **Every arrow label is on a dedicated segment outside nodes, with a visible 6–10px connector gap? No mask touches a node border or stroke?**
...
- [ ] Actual rendered text bounds fit their containers? Long text wrapped, resized, or given more space rather than judged from source coordinates?
...
## 11. Editing or Importing an Existing Diagram

Before editing or redrawing any source diagram (SVG, image, PDF, draw.io, Mermaid, or other format), create a **source fidelity ledger** of its nodes, connectors, labels, legend items, and semantic notes. Preserve every semantic item unless the user explicitly approves its removal; the complexity budget is not permission to drop source meaning. Recheck the finished diagram against the ledger.
...
For SVG output, render the artifact rather than trusting source coordinates. Inspect the whole diagram and 2×/3× crops of every region; a whole-image thumbnail is insufficient. Check actual text bounding boxes against containers, and wrap, resize, or expand nodes when text overflows.
