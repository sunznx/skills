...
# Move to user-level PATH directory
mv aliyun ~/bin/
export PATH="$HOME/bin:$PATH"  # Add to shell profile for persistence

# Verify
aliyun version
```

> **Note**: If you prefer a system-wide installation, add `$HOME/bin` to your PATH permanently by appending `export PATH="$HOME/bin:$PATH"` to your `~/.zshrc` or `~/.bash_profile`.
...
# Extract and install to user-level directory
