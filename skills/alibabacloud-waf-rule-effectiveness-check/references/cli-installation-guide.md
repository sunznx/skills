...
Download the archive first, inspect it, then install — no remote script execution, and the archive
integrity can be verified before extraction:

**x86_64 / amd64**
```bash
wget -q --connect-timeout=10 --read-timeout=120 --tries=3 https://aliyuncli.alicdn.com/aliyun-cli-macosx-latest-amd64.tgz
tar tzf aliyun-cli-macosx-latest-amd64.tgz        # list contents and confirm before extracting
...
**ARM64**
```bash
wget -q --connect-timeout=10 --read-timeout=120 --tries=3 https://aliyuncli.alicdn.com/aliyun-cli-macosx-latest-arm64.tgz
tar tzf aliyun-cli-macosx-latest-arm64.tgz
tar xzf aliyun-cli-macosx-latest-arm64.tgz
sudo mv aliyun /usr/local/bin/
```

On Linux, replace the archive URL with `https://aliyuncli.alicdn.com/aliyun-cli-linux-latest-amd64.tgz`
(or `...-arm64.tgz`).

**Alternative — official one-command installer** (executes a script fetched from the official Alibaba
