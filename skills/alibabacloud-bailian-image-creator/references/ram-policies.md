# RAM Policies for Bailian Image Creator
...
| modelstudio | `modelstudio:ListApiKeys` | List API keys (idempotent check before creation) |
| modelstudio | `modelstudio:CreateApiKey` | Create a DashScope API Key |
| modelstudio | `modelstudio:DeleteApiKey` | Delete a DashScope API Key (recycle old keys) |
...
- If you only need image generation (not API Key management), no RAM permissions are required -- only a valid DASHSCOPE_API_KEY
