...
> **Aliyun CLI 3.4.8+**: Supports installing and using all published Alibaba Cloud product plugins. Make sure to upgrade to 3.4.8 or later for full plugin ecosystem coverage.

## Installation

> **Project page**: https://github.com/aliyun/aliyun-cli
>
> **Actual installer download URL**: https://aliyuncli.alicdn.com/ (same as the [README Installation](https://github.com/aliyun/aliyun-cli/blob/master/README.md#installation) section)
>
> All scripts below use the latest stable build from aliyuncli.alicdn.com. Copy the block for your OS and run it directly.
...
> You can also use Homebrew: `brew install aliyun-cli`
...
> **Configuration note**
> The Dataphin plugin reuses the aliyun-cli credential system: **AccessKey/Endpoint are provided by the aliyun-cli main profile; tenant/project are provided by the plugin's own local profile**.
>
> **Configuration mode**
> - Public cloud SaaS: supports both interactive `aliyun configure` and non-interactive `aliyun configure set ...`. Only `--region` is required; no `--endpoint` is needed. Obtain the AccessKey from the [RAM console](https://ram.console.aliyun.com/manage/ak); a RAM sub-account AK/SK is recommended over the primary account.
> - Non-public cloud (standalone): supports **only non-interactive** `aliyun configure set ...` with `--endpoint` (management-plane OpenAPI address) and `--region cn-hangzhou`; using interactive `aliyun configure` fails with `Specified access key is not found`. Obtain the AccessKey from the Dataphin console via top-right avatar → AccessKey Management.
>
> **Troubleshooting**
> - Error: `Specified access key is not found`.
>   - Cause: standalone deployment used interactive configuration
>   - Solution: switch to non-interactive configuration for standalone deployments
...
1. **Install plugins** for services you need (v3.4.8+ supports all published product plugins):
   ```bash
