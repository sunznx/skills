...
> **⚠️ SECURITY WARNING**: The examples below use `<placeholder>` values. **NEVER** hardcode real credentials in scripts or documentation. Always obtain credentials from a secure source (e.g., environment variables, secrets manager). See [Security Best Practices](#security-best-practices) before proceeding.
...
aliyun configure set \
  ...
  --access-key-id <your-access-key-id> \
  --access-key-secret <your-access-key-secret> \
  --ram-role-arn acs:ram::<your-account-id>:role/<your-role-name> \
...
aliyun configure set --access-key-id <new-access-key-id> --access-key-secret <new-access-key-secret>
...
aliyun configure set --mode StsToken \
  --access-key-id <your-access-key-id> \
  --access-key-secret <your-access-key-secret> \
  --sts-token <new-sts-token> --region cn-hangzhou
```
