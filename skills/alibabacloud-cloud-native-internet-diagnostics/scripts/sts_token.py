...
# -*- coding: utf-8 -*-
# SECURITY:
#   - Read-only identity check: only sts:GetCallerIdentity is invoked.
#   - No credential carrying: this script never reads, prints, passes, or
#     caches AK/SK/STS tokens. Credentials are resolved exclusively by the
#     aliyun CLI default credential chain.
#   - The local cache (.sts_cache.json) stores only public identity facts
#     (AccountId / Arn / IdentityType) and is written with permission 0600.
...
Capabilities:
  ...
  - Cache the public identity facts in scripts/.sts_cache.json (chmod 0600)
...
# Public identity cache (no credentials); same directory as this script.
_CACHE_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ".sts_cache.json"
)
...
def cache_public_identity(identity: Dict[str, Any]) -> None:
    """Cache public identity facts (never credentials) with permission 0600."""
    payload = {
    ...
    try:
        ...
        # Cache failure is non-fatal: identity verification already succeeded.
        print(f"[WARN] identity cache write failed: {e}", file=sys.stderr)
...
def main():
    ...
    cache_public_identity(identity)
