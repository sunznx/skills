...
```bash
python3 "$SKILL_DIR/scripts/evaluation/agentloop_eval.py" discover --agent-space <space> --all-pages
```
...
```bash
python3 "$SKILL_DIR/scripts/evaluation/agentloop_eval.py" run --spec "$SKILL_DIR/references/evaluation/examples/oneshot-example.json"
```
...
python3 "$SKILL_DIR/scripts/evaluation/agentloop_eval.py" run \
  --spec "$SKILL_DIR/references/evaluation/examples/oneshot-example.json" --execute --output /tmp/eval-result.json
```
...
```bash
cd "$SKILL_DIR" && python3 -m unittest tests.evaluation.test_workflows -v
```
