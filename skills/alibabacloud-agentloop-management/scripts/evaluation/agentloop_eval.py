...
RAW_PROMPT_MODE = "raw_prompt"
# type=AGENT on its own is an Agent evaluator; adding agentEvaluatorMode=raw_prompt makes it an LLM judge.
RAW_PROMPT_LLM_CONFIG = {AGENT_EVALUATOR_MODE_KEY: RAW_PROMPT_MODE}
# rawPromptBackend is no longer part of the contract; stale spec keys are stripped before sending.
DEPRECATED_BACKEND_KEYS = (
    "rawPromptBackend",
    "raw_prompt_backend",
    "executionBackend",
    "execution_backend",
...
def normalize_llm_style_evaluator_config(config: Any) -> dict[str, Any]:
    ...
    normalized.pop("agent_evaluator_mode", None)
    for alias in DEPRECATED_BACKEND_KEYS:
        normalized.pop(alias, None)
    normalized.update(RAW_PROMPT_LLM_CONFIG)
    return normalized


def strip_deprecated_backend_keys(config: Any) -> Any:
    """Drop rawPromptBackend and its aliases; the backend no longer accepts them.

    Stale specs that still carry a backend key are cleaned rather than rejected, so the mode alone
    decides the behavior: absent means Agent, raw_prompt means LLM judge.
    """
    if not isinstance(config, dict):
        return config
    if not any(key in config for key in DEPRECATED_BACKEND_KEYS):
        return config
    normalized = copy.deepcopy(config)
    for key in DEPRECATED_BACKEND_KEYS:
...
def normalize_create_evaluator_definition(definition: dict[str, Any]) -> dict[str, Any]:
    ...
    if evaluator_type not in CREATABLE_EVALUATOR_TYPES:
        raise WorkflowError(
            ...
            f"config.agentEvaluatorMode='raw_prompt'; "
            f"got {evaluator_type!r}"
        )
    if evaluator_type == "AGENT" and "config" in normalized:
        normalized["config"] = strip_deprecated_backend_keys(normalized["config"])
