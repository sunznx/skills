...
CLI 会临时激活自己的 leaf，完成后恢复原活动 leaf；操作开始前还要等待插件内部 `mindMap` 实例初始化，不能只检查 `$bus`。若插件事件没有回包，`read` 使用该 fresh leaf 已加载的数据，不把事件超时误报为插件未运行。每次增量修改都等待插件当前数据反映预期变化后再保存；完成后再次 `read`，确认目标节点、父子关系和返回的新 UID。一次请求包含多个独立修改时，逐项执行并在最后验证；不要通过 Markdown 全量重建来代替局部编辑，以免丢失节点样式、链接和位置。
